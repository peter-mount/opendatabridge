...
import dev.area51.util.BlockingSupplier;
import dev.area51.util.BlockingSupplierConsumer;
...
/**
 * A {@link Supplier} which will provide received messages from a RabbitMQUtils Queue
 * <p>
 *
    ...
    implements BlockingSupplier<byte[]>
{
    ...
    RabbitSupplier( RabbitConnection connection,
                    String queueName,
                    String bindTopic,
                    ...
                    Map<String, Object> queueProperties )
    {
        ...
        if ( durable )
        {
            ...
            this.queueProperties.putIfAbsent( "x-message-ttl",
                                              DEFAULT_TTL );
        }
        else
        {
        ...
        thread = DaemonThreadFactory.INSTANCE.newThread( ( ) -> {
            ...
            while ( running )
            {
                ...
                catch ( IOException ex )
                {
                    if ( running )
                    {
                        LOG.log( Level.SEVERE,
                                 "IOException, attempting to recover",
                                 ex );
                        closeImpl( );
                    }
                }
                ...
                catch ( Throwable ex )
                {
                    ...
                    {
                        LOG.log( Level.WARNING,
                                 "Error whilst processing message, attempting to recover",
                                 ex );
                        closeImpl( );
                    }
                    else
                    {
                        LOG.log( Level.SEVERE,
                                 "Error whilst processing message, appears to be fatal?",
                                 ex );
                    }
                }
            }
            LOG.log( Level.FINE,
                     ( ) -> "Worker " + queueName + " terminated" );
            closeImpl( );
        ...
        IOException
    {
        ...
        LOG.log( Level.FINE,
                 ( ) -> "Creating consumer on " + queueName );
        ...
        if ( bindTopic != null )
        {
            ...
            if ( parked )
            {
                ...
                props.put( "x-message-ttl",
                           parkDelay );
                props.put( "x-dead-letter-exchange",
                           "" );
                props.put( "x-dead-letter-routing-key",
                           queueName );

                channel.queueDeclare( parkQueueName,
                                      durable,
                                      false,
                                      !durable,
                                      props );
            ...
            {
                channel.queueBind( parked ? parkQueueName : queueName,
                                   bindTopic,
                                   routingKey );
            }
        }
    ...
    public boolean equals( Object obj )
    {
        ...
        return Objects.equals( this.queueName,
                               other.queueName );
    }
